      ...
      {bookingList && bookingList.length > 0 ? (
        bookingList?.map((item, index) => (
          <div className=" flex gap-4 items-center border p-5 m-3 rounded-lg">
            <Image
              src={
                item.attributes?.stylist.data?.attributes?.image?.data
                  ?.attributes?.url
              }
            ...
            <div className="flex flex-col gap-2 w-full">
              <h2 className="font-bold text-[18px] items-center flex justify-between">
                {item.attributes.stylist.data.attributes.Name}
                {!expired && (
                  <CancelAppointment
              ...
              <h2 className="flex gap-2 text-gray-500">
                ...
                {item.attributes.stylist.data.attributes.Adsress}
              </h2>
              <h2 className="flex gap-2">
                <Calendar className="text-primary h-5 w-5" /> Appointment On:
                {moment(item.attributes.Pickdate).format("DD-MMM-YYYY")}{" "}
              </h2>
              <h2 className="flex gap-2">
